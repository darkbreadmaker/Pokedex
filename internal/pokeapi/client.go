...
func (c *Client) GetLocationArea(url string) LocationArea {
...
func (c *Client) ExploreLocation(url string) LocationData {
	...
	if err != nil {
		fmt.Errorf("Error unmashalling data: %s", err)
	}
	return locationData
}

func (c *Client) CatchPokemon(url string) Pokemon {
	pokemon :=  Pokemon{}
	value, ok := cache.Get(url)
	var body []byte
	if ok {
